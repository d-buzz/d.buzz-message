...
const encryptMessage = (memo_key_from, memo_key_to, message) => {
    let response = { data: null, error: null }
    try {
        const encodedMsg = hive.memo.encode(memo_key_from, memo_key_to, '# ' + message);
        if (encodedMsg) {
...
const getAccountHistory = async (account, start = -1, limit = 1000) => {
    let response = { data: null, error: null }
    try {
        let history = await dhiveClient().database.getAccountHistory(account, start, limit)
...
const getTransfers = async (account_from, memo_key_decrypted, account_to = '') => {
    let response = { data: null, error: null }
    let transfers = [];
    try {
        ...
        if (!history.data) {
            ...
            const result = transactions.filter(x => {
                ...
                return x[1].op[0] === 'transfer' && !account_to ? true : (xdata.from === account_to || xdata.to === account_to);
            })
